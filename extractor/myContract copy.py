...
import daikon.Daikon as daikon
...
class ContractStorageMonitor:
    ...
    def __init__(self, storageJson, slot=0, typeJson = None):
        ...
        # print("grid_storages", grid_storages)
        ...
        with alive_bar(min(LIMIT, len(statechanges)), force_tty=True) as bar:
            for tx_statechange in statechanges[:min(LIMIT, len(statechanges))]:
                ...
                if count>0 and len(enter)>0 and len(exit1)>0 and len(exit2)>0:
                    ...
                    enter.append("\n")
                    self.tx_blockNumber_index_before_after_contractStatesDataTraces[blockNumber_index] = [None, None, None]
                    if len(slot_statechanges)>0:
                        ...
                        self.tx_blockNumber_index_before_after_contractStatesDataTraces[blockNumber_index][0] = enter
                        self.tx_blockNumber_index_before_after_contractStatesDataTraces[blockNumber_index][1] = exit1
                    else:
                        ...
                        self.tx_blockNumber_index_before_after_contractStatesDataTraces[blockNumber_index][0] = enter
                        self.tx_blockNumber_index_before_after_contractStatesDataTraces[blockNumber_index][2] = exit2
                count += 1
                bar()
                if count > LIMIT:
                    break
